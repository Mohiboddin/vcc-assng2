...
void kvm_run_vm(struct vm *vm1, struct vm *vm2) {
    struct vm *vm;     
    int ret = 0;       
    int vm_select = 0; // 0 for VM1 (producer), 1 for VM2 (consumer)
    
    // Buffer to store produced values
    uint32_t producedValues[3] = {0};
    int producedCount = 0;  // Count of values produced in current burst
    int consumeIndex = 0;   // Index of next value to consume
    int burstComplete = 0;  // Flag to indicate if a burst of 3 productions is complete
    ...
    while (1) {
        // Select which VM to run
        vm = (vm_select == 0) ? vm1 : vm2;
        ret = ioctl(vm->vcpus->vcpu_fd, KVM_RUN, 0);
        ...
        if (vm->vcpus->kvm_run->exit_reason == KVM_EXIT_IO) {
            ...
            switch (vm->vcpus->kvm_run->io.port) {
                case 0x10:  // Producer (VM1) writing value
                    if (vm == vm1) {
                        // Store the produced value in our buffer
                        producedValues[producedCount] = *(uint32_t *)dataPtr;
                        printf("VMFD: %d Produced value: %d\n", vm->vm_fd, producedValues[producedCount]);
                        producedCount++;
                        
                        // Check if we've produced 3 values
                        if (producedCount == 3) {
                            burstComplete = 1;
                            vm_select = 1;  // Switch to consumer
                            producedCount = 0;  // Reset for next burst
                        }
                    }
                    sleep(1);
                    break;

                case 0x11:  // Consumer (VM2) reading value
                    if (vm == vm2) {
                        printf("VMFD: %d KVM_EXIT_IO\n", vm->vm_fd);
                        if (burstComplete) {
                            *(uint32_t *)dataPtr = producedValues[consumeIndex];
                        }
                    }
                    sleep(1);
                    break;

                case 0x12:  // Consumer (VM2) confirming consumption
                    if (vm == vm2) {
                        printf("VMFD: %d Consumed value: %d\n", vm->vm_fd, *(uint32_t *)dataPtr);
                        consumeIndex++;
                        
                        // Check if we've consumed all values from the burst
                        if (consumeIndex == 3) {
                            consumeIndex = 0;
                            burstComplete = 0;
                            vm_select = 0;  // Switch back to producer
                        }
                    }
                    sleep(1);
                    break;
