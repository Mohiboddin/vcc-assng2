...
{
    int produced_values[5];

    for (;;)
    {
        // Run the producer VM (vm1)
        ...
        if (vcpu1->kvm_run->exit_reason == KVM_EXIT_IO && vcpu1->kvm_run->io.port == 0xEA)
        {
            // Producer VM has trapped to the hypervisor
            // Read the base address of the array from the guest's memory
            uint32_t base_address;
            memcpy(&base_address, &vm1->mem[vcpu1->kvm_run->io.data_offset], sizeof(base_address));

            // Copy the produced values from the producer VM's memory
            memcpy(produced_values, &vm1->mem[base_address], sizeof(produced_values));

            // Print the produced values
            printf("VMFD: %d Produced Values: %d %d %d %d %d\n", vm1->vm_fd, produced_values[0], produced_values[1], produced_values[2], produced_values[3], produced_values[4]);

            // Copy the produced values to the consumer VM's memory
            memcpy(&vm2->mem[base_address], produced_values, sizeof(produced_values));

            // Run the consumer VM (vm2)
            ...
            if (vcpu2->kvm_run->exit_reason == KVM_EXIT_IO && vcpu2->kvm_run->io.port == 0xEA)
            {
                // Consumer VM has trapped to the hypervisor
                // Read the consumed value from the I/O port
                uint32_t consumed_value;
                memcpy(&consumed_value, &vm2->mem[vcpu2->kvm_run->io.data_offset], sizeof(consumed_value));

                // Print the consumed value
                printf("VMFD: %d Consumed Value: %d\n", vm2->vm_fd, consumed_value);
            }
        }
    }
